...
/* أسلوب موحّد */
const S = "#3d3357"; // لون الحدود
const K = `stroke="${S}" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"`;
const SKIN = "#f7c8a0";
const CREAM = "#fbf7ef";

const line2 = (x1, y1, x2, y2, c, w = 12) =>
  `<path d="M${x1} ${y1} L${x2} ${y2}" stroke="${S}" stroke-width="${w + 7}" stroke-linecap="round" fill="none"/>` +
  `<path d="M${x1} ${y1} L${x2} ${y2}" stroke="${c}" stroke-width="${w}" stroke-linecap="round" fill="none"/>`;

const path2 = (d, c, w = 12) =>
  `<path d="${d}" stroke="${S}" stroke-width="${w + 7}" stroke-linecap="round" fill="none"/>` +
...
const WORDS = [
  { id: 1, slug: "dresha", alt: "نافذة", colors: ["#f4a259", "#5c9ded", "#6bbf59"],
    draw: (c) =>
      `<rect x="45" y="30" width="110" height="135" rx="12" fill="${c}" ${K}/>` +
      `<rect x="59" y="44" width="82" height="107" rx="6" fill="#c9e8ff" ${K}/>` +
      `<line x1="100" y1="46" x2="100" y2="149" ${K}/>` +
      `<line x1="61" y1="98" x2="139" y2="98" ${K}/>` +
      `<circle cx="78" cy="66" r="9" fill="#fff" opacity="0.8"/>` },

  { id: 2, slug: "hoosh", alt: "فناء البيت", needsCustom: true, colors: ["#ef6461", "#5c9ded", "#6bbf59"],
    draw: (c) =>
  ...
  { id: 3, slug: "mkhadda", alt: "وسادة", colors: ["#a06cd5", "#4ecdc4", "#f28ab2"],
    draw: (c) =>
  ...
  { id: 4, slug: "doolab", alt: "خزانة", colors: ["#b08968", "#5c9ded", "#4ecdc4"],
    draw: (c) =>
  ...
  { id: 5, slug: "gooti", alt: "علبة", colors: ["#ef6461", "#6bbf59", "#f4a259"],
    draw: (c) =>
      `<rect x="63" y="52" width="74" height="110" rx="12" fill="${c}" ${K}/>` +
      `<rect x="63" y="86" width="74" height="38" fill="#fff" ${K}/>` +
      `<ellipse cx="100" cy="52" rx="37" ry="13" fill="#dfe7ee" ${K}/>` +
      `<ellipse cx="100" cy="52" rx="20" ry="6" fill="#c3ced8" stroke="${S}" stroke-width="3"/>` },

  { id: 6, slug: "istikana", alt: "استكانة شاي", needsCustom: true, colors: ["#4ecdc4", "#ef6461", "#a06cd5"],
    draw: (c) =>
  ...
  { id: 7, slug: "mallas", alt: "ملعقة", colors: ["#9aa5b1", "#f4a259", "#4ecdc4"],
    draw: (c) =>
  ...
  { id: 8, slug: "jidir", alt: "قِدر", colors: ["#5c9ded", "#ef6461", "#9aa5b1"],
    draw: (c) =>
      ...
      `<ellipse cx="100" cy="80" rx="54" ry="13" fill="#dfe7ee" ${K}/>` +
  ...
  { id: 9, slug: "junta", alt: "حقيبة مدرسية", needsCustom: true, colors: ["#ef6461", "#5c9ded", "#6bbf59"],
    draw: (c) =>
  ...
  { id: 10, slug: "barraya", alt: "مبراة", colors: ["#6bbf59", "#ef6461", "#5c9ded"],
    draw: (c) =>
      ...
      `<rect x="8" y="100" width="62" height="17" rx="4" fill="#f7cf5c" ${K}/>` +
  ...
  { id: 11, slug: "daftar", alt: "دفتر", colors: ["#f4a259", "#a06cd5", "#4ecdc4"],
    draw: (c) =>
      `<rect x="58" y="38" width="88" height="124" rx="10" fill="${c}" ${K}/>` +
      [0, 1, 2, 3].map((i) => `<circle cx="58" cy="${58 + i * 28}" r="6" fill="#fff" ${K}/>`).join("") +
      `<line x1="78" y1="72" x2="132" y2="72" stroke="#fff" stroke-width="5" stroke-linecap="round"/>` +
      `<line x1="78" y1="98" x2="132" y2="98" stroke="#fff" stroke-width="5" stroke-linecap="round"/>` +
      `<line x1="78" y1="124" x2="118" y2="124" stroke="#fff" stroke-width="5" stroke-linecap="round"/>` },

  { id: 12, slug: "dishdasha", alt: "دشداشة كويتية", needsCustom: true, colors: ["#4ecdc4", "#f4a259", "#a06cd5"],
    draw: (c) =>
  ...
  { id: 13, slug: "jooti", alt: "حذاء رياضي", colors: ["#5c9ded", "#ef6461", "#6bbf59"],
    draw: (c) =>
  ...
  { id: 14, slug: "ghitra", alt: "غترة وعقال", needsCustom: true, colors: ["#f4a259", "#4ecdc4", "#a06cd5"],
    draw: (c) =>
  ...
  { id: 15, slug: "naal", alt: "نعال", needsCustom: true, colors: ["#6bbf59", "#5c9ded", "#f4a259"],
    draw: (c) =>
  ...
  { id: 16, slug: "gattu", alt: "قطة", colors: ["#f4a259", "#9aa5b1", "#b08968"],
    draw: (c) =>
  ...
  { id: 17, slug: "diyaya", alt: "دجاجة", colors: ["#f4a259", "#b08968", "#ef6461"],
    draw: (c) =>
      `<path d="M78 44 Q74 28 86 32 Q86 20 96 26 Q100 14 106 28" fill="#ef6461" ${K}/>` +
      `<path d="M58 96 C58 62 116 52 138 74 C158 94 146 144 104 148 C70 150 58 122 58 96 Z" fill="#fff" ${K}/>` +
      `<path d="M60 78 L44 84 L60 92 Z" fill="#f4a259" ${K}/>` +
      `<circle cx="76" cy="72" r="4" fill="${S}"/>` +
      `<path d="M96 96 C112 88 132 96 128 112 C124 126 100 124 96 96 Z" fill="${c}" ${K}/>` +
      line2(96, 148, 92, 168, "#f4a259", 6) + line2(116, 148, 118, 168, "#f4a259", 6) },

  { id: 18, slug: "baeer", alt: "جمل", colors: ["#e0b880", "#c99e66", "#b08968"],
    draw: (c) =>
  ...
  { id: 19, slug: "simcha", alt: "سمكة", colors: ["#5c9ded", "#ef6461", "#f4a259"],
    draw: (c) =>
  ...
  { id: 21, slug: "aish", alt: "صحن عيش", colors: ["#4ecdc4", "#ef6461", "#a06cd5"],
    draw: (c) =>
  ...
  { id: 22, slug: "mai", alt: "كوب ماء", colors: ["#5c9ded", "#4ecdc4", "#a06cd5"],
    draw: (c) =>
  ...
  { id: 23, slug: "rayoog", alt: "ريوق — بيضة", colors: ["#f4a259", "#4ecdc4", "#f28ab2"],
    draw: (c) =>
      ...
      `<circle cx="102" cy="96" r="15" fill="#f7cf5c" ${K}/>` },

  { id: 24, slug: "motar", alt: "سيارة", colors: ["#ef6461", "#5c9ded", "#f7cf5c"],
    draw: (c) =>
      `<path d="M52 112 L60 88 C64 76 76 70 92 70 L114 70 C130 70 140 78 146 90 L154 112 Z" fill="${c}" ${K}/>` +
      `<path d="M70 88 C74 79 82 76 92 76 L96 76 L96 108 L64 108 Z" fill="#c9e8ff" ${K}/>` +
      `<path d="M104 76 L112 76 C124 76 132 82 136 90 L140 108 L104 108 Z" fill="#c9e8ff" ${K}/>` +
  ...
  { id: 25, slug: "seekal", alt: "دراجة", colors: ["#6bbf59", "#ef6461", "#5c9ded"],
    draw: (c) =>
  ...
  { id: 26, slug: "freej", alt: "فريج — بيوت", needsCustom: true, colors: ["#f4a259", "#4ecdc4", "#f28ab2"],
    draw: (c) =>
      ...
      `<rect x="42" y="112" width="16" height="14" fill="#c9e8ff" ${K}/><rect x="140" y="112" width="16" height="14" fill="#c9e8ff" ${K}/>` },

  { id: 27, slug: "darb", alt: "طريق", colors: ["#9aa5b1", "#8d99ae", "#a5b0bc"],
    draw: (c) =>
      `<circle cx="156" cy="48" r="18" fill="#f7cf5c" ${K}/>` +
      ...
      `<circle cx="38" cy="150" r="12" fill="#6bbf59" ${K}/><circle cx="166" cy="140" r="10" fill="#6bbf59" ${K}/>` },

  { id: 28, slug: "yadda", alt: "جدة", colors: ["#a06cd5", "#4ecdc4", "#ef6461"],
    draw: (c) =>
  ...
  { id: 29, slug: "yumma", alt: "أم", colors: ["#f28ab2", "#4ecdc4", "#f4a259"],
    draw: (c) =>
      ...
      face(100, 102) +
      `<circle cx="79" cy="108" r="6" fill="#ef6461" opacity="0.35"/><circle cx="121" cy="108" r="6" fill="#ef6461" opacity="0.35"/>` },

  { id: 30, slug: "iyal", alt: "أطفال", colors: ["#5c9ded", "#f28ab2", "#6bbf59"],
    draw: (c) =>
      ...
      face(132, 76) +
      `<path d="M108 160 L112 116 C114 104 124 98 132 98 C140 98 150 104 152 116 L156 160 Z" fill="#f7cf5c" ${K}/>` +
      `<line x1="92" y1="126" x2="108" y2="126" stroke="${SKIN}" stroke-width="9" stroke-linecap="round"/>` },

  { id: 31, slug: "baby", alt: "طفل رضيع", colors: ["#f28ab2", "#5c9ded", "#6bbf59"],
    draw: (c) =>
      ...
      `<circle cx="72" cy="102" r="7" fill="#ef6461" opacity="0.35"/><circle cx="128" cy="102" r="7" fill="#ef6461" opacity="0.35"/>` +
  ...
  { id: 32, slug: "rassasi", alt: "اللون الرصاصي", colors: ["#9aa5b1", "#9aa5b1", "#9aa5b1"],
    draw: (c) =>
  ...
  { id: 33, slug: "samawi", alt: "اللون السماوي", colors: ["#8fd0ff", "#8fd0ff", "#8fd0ff"],
    draw: (c) =>
      `<path d="M100 40 C132 34 164 62 156 94 C172 106 158 138 132 132 C126 158 90 166 76 144 C50 150 36 122 50 106 C34 88 54 52 80 58 C84 46 92 42 100 40 Z" fill="${c}" ${K}/>` +
      `<circle cx="150" cy="150" r="9" fill="${c}" ${K}/><circle cx="52" cy="58" r="7" fill="${c}" ${K}/>` +
      `<ellipse cx="86" cy="84" rx="18" ry="10" fill="#fff" opacity="0.45"/>` },

  { id: 34, slug: "wardi", alt: "اللون الوردي", colors: ["#f28ab2", "#f28ab2", "#f28ab2"],
    draw: (c) =>
      `<path d="M100 40 C132 34 164 62 156 94 C172 106 158 138 132 132 C126 158 90 166 76 144 C50 150 36 122 50 106 C34 88 54 52 80 58 C84 46 92 42 100 40 Z" fill="${c}" ${K}/>` +
      `<circle cx="150" cy="150" r="9" fill="${c}" ${K}/><circle cx="52" cy="58" r="7" fill="${c}" ${K}/>` +
  ...
  { id: 35, slug: "shlonak", alt: "تحية — يد تلوّح", colors: [SKIN, SKIN, SKIN],
    draw: (c) =>
      ...
      `<path d="M84 156 L120 156 C124 148 122 140 118 136 L86 136 C80 140 80 150 84 156 Z" fill="#4ecdc4" ${K}/>` },

  { id: 36, slug: "wayed", alt: "نجوم كثيرة", colors: ["#f7cf5c", "#f7cf5c", "#f7cf5c"],
    draw: (c) =>
      star(100, 92, 44, c) +
  ...
  { id: 37, slug: "zain", alt: "إبهام لأعلى", colors: [SKIN, SKIN, SKIN],
    draw: (c) =>
      `<path d="M52 60 L60 46 M100 40 L100 26 M144 58 L152 46" stroke="#f7cf5c" stroke-width="6" stroke-linecap="round"/>` +
      `<rect x="52" y="98" width="26" height="60" rx="8" fill="#4ecdc4" ${K}/>` +
  ...
  { id: 38, slug: "yalla", alt: "طفل يركض", colors: ["#5c9ded", "#ef6461", "#6bbf59"],
    draw: (c) =>
      `<path d="M26 78 L48 78 M20 100 L44 100 M28 122 L48 122" stroke="${S}" stroke-width="5" stroke-linecap="round" opacity="0.4"/>` +
      ...
      `<ellipse cx="70" cy="150" rx="14" ry="8" fill="#ef6461" ${K}/>` +
      `<ellipse cx="144" cy="156" rx="14" ry="8" fill="#ef6461" ${K}/>` },
  ...
  { id: 39, slug: "chithee", alt: "إصبع يشير", colors: [SKIN, SKIN, SKIN],
    draw: (c) =>
      ...
      `<path d="M70 40 L60 32 M92 26 L92 16 M112 38 L120 30" stroke="#f7cf5c" stroke-width="5" stroke-linecap="round"/>` +
      `<path d="M84 160 L120 160 C124 152 122 148 118 144 L86 144 C82 148 80 154 84 160 Z" fill="#a06cd5" ${K}/>` },

  { id: 40, slug: "mashkoor", alt: "قلب شكر", colors: ["#ef6461", "#f28ab2", "#f4a259"],
    draw: (c) =>
      `<path d="M100 152 C36 110 52 48 100 76 C148 48 164 110 100 152 Z" fill="${c}" ${K}/>` +
      star(48, 48, 12, "#f7cf5c") + star(156, 52, 14, "#f7cf5c") + star(100, 26, 10, "#f7cf5c") +
      `<ellipse cx="80" cy="92" rx="12" ry="7" fill="#fff" opacity="0.4" transform="rotate(-25 80 92)"/>` },
];

/* ── تجميع الملفات ── */
const shadow = `<ellipse cx="100" cy="180" rx="54" ry="9" fill="${S}" opacity="0.08"/>`;
...
<circle cx="76" cy="82" r="12" fill="#f7cf5c" ${K}/>
<path d="M52 140 L88 104 L110 126 L128 108 L148 140 Z" fill="#9aa5b1" ${K}/>
</svg>\n`
);
